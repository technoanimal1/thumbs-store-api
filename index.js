...
// Rewrite storage_url's ?format=avif to the client's preferred format.
// Same underlying PNG file on disk — different Supabase Image Transforms render.
// Returns the URL unchanged when preferred is falsy, "avif", or the URL has no format=avif param.
function formatThumbnail(url, preferred) {
if (!url || !preferred || preferred === 'avif') return url;
return url.replace(/([?&])format=avif/, `$1format=${preferred}`);
}
// ─── Helpers ───────────────────────────────────────────────────────────────────
...
synced_at,
providers ( slug, name ),
figma_games ( storage_url, published_at, category )
)
`)
...
const fmt = req.client.preferred_format;
res.json({
...
type: m.figma_games?.category || m.game_type || "slots",
thumbnail_url: formatThumbnail(m.figma_games?.storage_url || null, fmt),
...
try {
const c = req.client;
const fmt = c.preferred_format;

// If a provider filter was passed, validate it's licensed for this client BEFORE
// hitting the games view — otherwise unknown/unauthorized slugs silently return
// an empty payload instead of a helpful 404.
if (req.query.provider) {
const { data: cpRows } = await supabase
.from("client_providers")
.select("providers(slug)")
.eq("client_id", c.id);
const licensed = new Set((cpRows || []).map(r => r.providers?.slug).filter(Boolean));
if (!licensed.has(req.query.provider)) {
return res.status(404).json({
error: `unknown or unauthorized provider: ${req.query.provider}`,
licensed_providers: Array.from(licensed).sort()
});
}
}
...
type: r.category || r.slotegrator_type || "slots",
thumbnail_url: formatThumbnail(r.storage_url || null, fmt),
...
.from("catalog_mappings")
.select("id, game_name, slotegrator_uuid, slotegrator_type, game_type, catalog_slug, provider_id, figma_game_id")
.or(`id.eq.${req.params.game_id},slotegrator_uuid.eq.${req.params.game_id},catalog_slug.eq.${req.params.game_id}`)
.maybeSingle();
...
.select("slug,storage_url,published_at,category").eq("id", cmRow.figma_game_id).maybeSingle()
: { data: null };
res.json({
id: cmRow.slotegrator_uuid || cmRow.id,
provider_slug: prov ? prov.slug : null,
catalog_slug: cmRow.catalog_slug || null,
name: cmRow.game_name,
slug: fg ? fg.slug : null,
type: (fg && fg.category) || cmRow.game_type || cmRow.slotegrator_type || "slots",
provider: prov ? {
slug: prov.slug,
name: prov.name,
code: (typeof providerCodes !== "undefined" && providerCodes[prov.slug]) || null
} : null,
thumbnail_url: fg ? formatThumbnail(fg.storage_url, fmt) : null,
