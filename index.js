...
/** Build both PNG and AVIF URLs from a storage_url */
function buildThumbnailUrls(storageUrl, preferredFormat) {
  if (!storageUrl) return { thumbnail_url: null, thumbnail_png: null, thumbnail_avif: null };

  // Always derive a clean PNG base URL
  const pngUrl = storageUrl
    .replace('/render/image/public/', '/object/public/')
    .replace(/\?.*$/, '');

  const avifUrl = pngUrl
    .replace('/object/public/', '/render/image/public/') + '?format=avif&quality=75';

  if (preferredFormat === 'all') {
    return {
      thumbnail_url:  avifUrl,
      thumbnail_png:  pngUrl,
      thumbnail_avif: avifUrl,
    };
  }
  if (preferredFormat === 'png') {
    return { thumbnail_url: pngUrl, thumbnail_png: pngUrl, thumbnail_avif: null };
  }
  // default: avif
...
app.get("/api/thumbnails", requireApiKey, async (req, res) => {
  ...
  res.json({
    ...
    results: (data || []).map(row => {
      ...
      const urls = buildThumbnailUrls(m.figma_games?.storage_url, req.client.preferred_format);
      return {
